...
func (this *Collection) Map(handler interface{}) *Collection {
	...
	default:
		...
		if numOut > 0 {
			newCollection := &Collection{mapData: nil, array: make([]interface{}, 0)}
			for index, data := range this.array {
				newCollection.array = append(newCollection.array, handlerValue.Call(argsGetter(index, data))[0].Interface())
			}
